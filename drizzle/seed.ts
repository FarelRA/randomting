...
async function seed() {
  ...
  const existingAdmin = await db.select().from(schema.users).where(eq(schema.users.email, 'admin@randomting.app')).get()
  if (!existingAdmin) {
  ...
  console.log(`Seeded ${jokes.length} jokes, ${facts.length} facts, ${quotes.length} quotes`)
}
